 ...
 */
function check_login($dbc, $fname = '', $pass = '') {
	...
	if (empty($fname)) {
		...
		$n = mysqli_real_escape_string($dbc, trim($fname));
	}
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id, first_name FROM users WHERE first_name='$n' AND password=SHA1('$p')";		
